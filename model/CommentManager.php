...
class CommentManager extends Manager
{
    ...
    {
        $db = $this->dbConnect();
        $comments = $db->prepare('SELECT c.author, c.id AS com_id, c.comment, c.validated, c.like_count, c.dislike_count, c.comment_date, p.title
                              FROM comments c
                              INNER JOIN posts p
                              ON c.post_id = p.id
    ...
    {
        $db = $this->dbConnect();
        $comments = $db->prepare('SELECT c.author, c.id AS com_id, c.comment, c.like_count, c.dislike_count, c.comment_date, p.title
                                FROM comments c
                                INNER JOIN posts p
                                ON c.post_id = p.id
                                WHERE c.validated = "0"
    ...
    public function postComment()
    {
        ...
        $comments = $db->prepare('INSERT INTO comments(post_id, author, comment, validated, like_count, dislike_count, comment_date) VALUES(?, ?, ?, "0", "0", "0", NOW())');
